...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['toggle_role'])) {
    ...
    if ($id !== (int)$_SESSION['user_id']) {
        ...
        catat_log($pdo, (int)$_SESSION['user_id'], 'Ubah role pengguna', "ID: $id");
        $flash = 'Role pengguna berhasil diubah.';
    } else {
        $flash = 'Tidak bisa mengubah role akun sendiri.';
    }
}
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['toggle_active'])) {
    ...
    $flash = 'Status pengguna berhasil diperbarui.';
}

$daftar = $pdo->query("SELECT * FROM users ORDER BY role DESC, created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<title>Kelola Pengguna - HikeGear Admin</title>
<style>
...
.role-admin{background:#f1e9fb;color:#8e44ad}
...
<div class="layout">
  <aside class="sidebar">
    ...
    <a href="admin_pengguna.php" class="nav-link active"><span class="ic">👤</span>Pengguna</a>
  ...
  <div class="admin-main">
    ...
    <div class="content">
      ...
      <div class="sub">Semua akun (admin & pelanggan), atur role dan status</div>
      ...
      <table>
        ...
        <tbody>
          <?php if (empty($daftar)): ?>
          <tr><td colspan="6" style="text-align:center;padding:20px;color:var(--gray-600)">Belum ada pengguna.</td></tr>
          <?php else: foreach ($daftar as $u): ?>
          <tr>
            <td><b><?= htmlspecialchars($u['nama_lengkap']) ?></b></td>
            ...
            <td>
              ...
              <form method="POST" style="display:inline" onsubmit="return confirm('Ubah role pengguna ini?')">
                <input type="hidden" name="id" value="<?= $u['id'] ?>">
                <button type="submit" name="toggle_role" class="toggle-btn" style="background:#f1e9fb;color:#8e44ad">
              ...
              <?php else: ?>
              <span style="color:var(--gray-600);font-size:11.5px">Akun Anda</span>
              <?php endif; ?>
            </td>
          </tr>
