...
$daftar = $pdo->query("SELECT l.*, u.nama_lengkap AS nama_admin
...
<title>Log Aktivitas - HikeGear Admin</title>
<style>
...
.log-item{display:flex;gap:12px;padding:14px 18px;border-bottom:1px solid var(--gray-100)}
.log-item:last-child{border:none}
.log-dot{width:8px;height:8px;border-radius:50%;background:var(--accent);margin-top:6px;flex-shrink:0}
.log-text .aksi{font-weight:700;color:var(--gray-800);font-size:13px}
.log-text .ket{color:var(--gray-600);font-size:12px}
.log-text .waktu{color:var(--gray-400,#9ab0a4);font-size:11px;margin-top:2px}
</style>
</head>
<body>
<div class="layout">
  <aside class="sidebar">
    ...
    <a href="admin_log.php" class="nav-link active"><span class="ic">📜</span>Log Aktivitas</a>
  ...
  <div class="admin-main">
    ...
    <div class="content">
      <h1>📜 Log Aktivitas</h1>
      <div class="sub">Riwayat aktivitas admin (100 terakhir)</div>

      <div style="background:white;border-radius:var(--radius);box-shadow:0 1px 3px rgba(0,0,0,.06)">
        <?php if (empty($daftar)): ?>
        <div style="padding:30px;text-align:center;color:var(--gray-600)">Belum ada aktivitas tercatat.</div>
        <?php else: foreach ($daftar as $l): ?>
        <div class="log-item">
          <div class="log-dot"></div>
          <div class="log-text">
            <div class="aksi"><?= htmlspecialchars($l['aksi']) ?> <span style="font-weight:400;color:var(--gray-600)">oleh <?= htmlspecialchars($l['nama_admin'] ?? 'Sistem') ?></span></div>
            <?php if ($l['keterangan']): ?><div class="ket"><?= htmlspecialchars($l['keterangan']) ?></div><?php endif; ?>
            <div class="waktu"><?= date('d M Y, H:i', strtotime($l['created_at'])) ?> WIB</div>
          </div>
        </div>
